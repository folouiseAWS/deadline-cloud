...
"""
Shared combo box widgets for selecting AWS Deadline Cloud resources
(farms, queues, storage profiles).

These widgets are used by both the Settings dialog (DeadlineWorkstationConfigWidget)
and the Submit dialog (DeadlineCloudSettingsWidget).
"""
...
from qtpy.QtCore import QSize as _QSize, Signal as _Signal
...
from .._utils import CancelationFlag as _CancelationFlag, block_signals as _block_signals


class _DeadlineResourceListComboBox(_QWidget):
    """
    ...
    background_exception = _Signal(str, BaseException)

    # Emitted when an async refresh_farms_list thread completes,
    # provides (refresh_id, [(farm_id, farm_name), ...])
    _list_update = _Signal(int, list)

    def __init__(self, resource_name, setting_name, parent: _Optional[_QWidget] = None):
        super().__init__(parent)

        self.__refresh_thread = None
        self.__refresh_id = 0
        self.canceled = _CancelationFlag()
    ...
    def _build_ui(self):
        self.box = _QComboBox(parent=self)
        ...
        self.refresh_button.setIcon(_QApplication.style().standardIcon(_QStyle.SP_BrowserReload))
        self.refresh_button.setFixedSize(_QSize(22, 22))  # Make the button square
    ...
    def set_config(self, config: _ConfigParser) -> None:
    ...
    def refresh_list(self):
        ...
        """
        config = self.config
        selected_id = _config_file.get_setting(self.setting_name, config=config)
        ...
        self.__refresh_id += 1
        self.__refresh_thread = _threading.Thread(
    ...
    def refresh_selected_id(self):
        ...
        selected_id = _config_file.get_setting(self.setting_name, config=self.config)
    ...
    def _refresh_thread_function(self, refresh_id: int, config: _Optional[_ConfigParser] = None):
        """
...
class DeadlineFarmListComboBox(_DeadlineResourceListComboBox):
    def __init__(self, parent: _Optional[_QWidget] = None):
        super().__init__(resource_name="Farm", setting_name="defaults.farm_id", parent=parent)

    def list_resources(self, config: _Optional[_ConfigParser]):
        response = _api.list_farms(config=config)
        return sorted(
...
class DeadlineQueueListComboBox(_DeadlineResourceListComboBox):
    ...
    def list_resources(self, config: _Optional[_ConfigParser]):
        default_farm_id = _config_file.get_setting("defaults.farm_id", config=config)
        if default_farm_id:
            response = _api.list_queues(config=config, farmId=default_farm_id)
            return sorted(
...
class DeadlineStorageProfileNameListComboBox(_DeadlineResourceListComboBox):
    ...
    def list_resources(self, config: _Optional[_ConfigParser]):
        default_farm_id = _config_file.get_setting("defaults.farm_id", config=config)
        default_queue_id = _config_file.get_setting("defaults.queue_id", config=config)
        if default_farm_id and default_queue_id:
            response = _api.list_storage_profiles_for_queue(
    ...
    def _get_current_os(self) -> str:
        """
        Get a string specifying what the OS is, following the format
        the Deadline storage profile API expects.
        """
